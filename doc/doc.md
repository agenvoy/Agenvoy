...
| `/session` `/new`          | Switch to another session by self id (`d` deletes it), or create one; the `/session` list also edits the current session `name` / `id` / `role` (`/session name\|id\|role` opens one directly) |
