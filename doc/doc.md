...
| `TYPESAFE_API_KEY` | TypeSafe/Jev (beta) dispatcher and auto reasoning; create one in the [TypeSafe Console](https://console.typesafe.ai/keys) |
...
In **Config › Model**, each API-key provider card links to its key page (`Create one in the <provider> Console.`) and each OAuth card links to its subscription page (`Subscribe on the <provider> Plans page.`). Provider model lists and the image, STT and TTS model lists are cached in ToriiDB for 15 minutes. A provider's cached lists are cleared when its key is saved or its OAuth login completes or is cleared.
...
| `dispatcher_beta`                   |   `false` | Route with TypeSafe/Jev instead of the dispatcher model; see [TypeSafe/Jev (beta)](#typesafejev-beta) |
| `auto_reasoning`                    |   `false` | Let TypeSafe/Jev pick the reasoning level per request; see [TypeSafe/Jev (beta)](#typesafejev-beta) |
...
`output_dir` expands `~` and creates the directory when it is saved; a path that cannot be created is rejected, and one that later becomes unusable falls back to the default. **Config › System** applies it to the running daemon at once. The same tab shows the running version beside the latest release and, only when they differ, an **update** button that opens a terminal running `agen update`. Its License card links the corresponding source on GitHub. The TUI's `/config` (reply language, output dir) writes `config.json` the same way a hand edit does.
...
| --- | --- | --- | --- |
| `code` | Writing, fixing, debugging or testing code; requests that ask outright for depth or precision | S>A>B>C | `xhigh` |
| `chat` | Greetings, short answers, translation | B>C>A>S | `none` |
| `fetch` | Fetching data with tools and returning it without judgement | C>B>A>S | `low` |
...
| Command                    | Purpose                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/model`                   | Pick the session model (`auto` or a registered model; `d` removes the highlighted model, `t` sets its tier); `add` a provider; set the dispatch (a model or `TypeSafe/Jev(beta)`), auto `reasoning`, summary, image, STT and TTS settings                                                                                                                                 |
...
| `/session` `/new`          | Switch to another session by self id (`d` deletes it), or create one; the `/session` list also edits the current session `name` / `self id` / `role` (`/session name\|id\|role` opens one directly)                                                                                                                                                                       |
...
| `/skill`                   | Two tabs. `permission`: Enter toggles the highlighted skill between `always allow` / `ask`; `always allow` skills skip the permission prompt globally, and `o` opens the highlighted skill's folder. `system`: multi-select official skills; checked clones `github.com/agenvoy/skill-<name>` into `~/.config/agenvoy/skills/.system_design/<name>`, unchecked removes it |
| `/rule`                    | List, add or edit rules (title + description)                                                                                                                                                                                                                                                                                                                             |
...
| `Shift+A` / `Shift+D` | Cycle the reasoning level (inactive while auto reasoning is on) |
...
`/v1/chat/completions` is OpenAI-compatible and stateless: include prior messages in every request when continuity is needed. `reasoning_effort` accepts `none` `low` `medium` `high` `xhigh` `max` (plus the aliases `minimal` `extra` `ultra`); omitted or unrecognized values use the auto reasoning level when [auto reasoning](#typesafejev-beta) is on, otherwise `medium`.
...
| `DELETE`       | `/v1/key`         | **local** — delete a single credential (`?key=`) from the keychain and drop it from the recorded key list. There is no read counterpart: credentials are only written and listed by name. |
| `GET` `POST`   | `/v1/keys`        | **local** — list the recorded credential names, or store one (`{key, value}`) in the keychain.                           |
...
| `GET`    | `/v1/provider/:provider/models`  | **local** — list models available to this provider. The response carries `windows`, a map of model id to `{in, out}` context sizes resolved from `llm-io.agenvoy.com`; models the catalog does not cover are omitted. For a local / custom endpoint name (`ollama`, `llama.cpp`, or a recorded one) it asks the endpoint's `GET /models`, returning 502 when the endpoint does not answer. The list is cached for 15 minutes.                                                                                                              |
| `GET`    | `/v1/provider/:provider/console` | **local** — redirect (302) to the provider's console: `?page=key` for API keys or `plan` for the subscription; defaults to `plan` when the provider has one, otherwise `key`; 404 when that page does not exist.                                                                                                                                                                                                                                                                                                                           |
...
| Files        | `find_files`                      | Locate by directory, name pattern or content (`mode=list\|glob\|search`); glob lists the most recently modified first; search is paged by `offset`/`limit` (256 per page) and returns either matching paths with counts and their first five matching rows (`output=files`) or numbered matching lines with optional `context` (`output=content`); `multiline` lets the regex span lines |
|              | `read_files`                      | Batch-read text, PDF, DOCX, PPTX, CSV or images; 2048 lines by default, and a text file cut short ends with the next `offset`; `around` reads `context` lines (20 by default) on both sides of given rows of a text file; one path may repeat in a call                                                                                                                                  |
|              | `edit_file`                       | Create, edit, move aside or restore a file (`mode=write\|patch\|remove\|restore`); overwriting or patching an existing file requires a `read_files` of it earlier in the same run, with the file unchanged on disk since                                                                                                                                                                 |
...
|              | `run_command_readonly`            | Same sandbox, for read-only inspection the file tools do not cover (`git status`/`log`/`diff`, `du`, `which`); runs without confirmation and in parallel with other calls. Reading, listing and grepping stay with `read_files` / `find_files`. Nothing enforces the read-only part beyond the tool description and the sandbox, and `cd` is refused                                     |
...
|              | `ask_user`                        | Pause and ask. In the TUI the question is inline: the run stays alive with no tool timeout and continues on the answer. On every other channel the run stops and its state is snapshotted as a pending task that the answer resumes (args are kept to 1 KiB and tool results to 4 KiB in that snapshot)                                                                                                                                                                                                                                                                                                                                            |
...
Fifteen tools ship with full schemas — `ask_user`, `calculate`, `chat_history`, `edit_file`, `fetch_page`, `find_files`, `find_tools`, `read_files`, `reasoning_guide`, `run_command`, `run_command_readonly`, `run_skill`, `search_web`, `write_result`, `write_todo`. Everything else arrives as a name and a description; its parameters load on first use through `find_tools(mode=search)`, keeping the initial tool payload well under the full registry. The `edit_file` patch mode accepts only `{old_string, new_string}` targets (plus optional `replace_all`); `new_string` replaces `old_string`, and insertion is expressed by repeating `old_string` at the start of `new_string`. Every target is matched against the file as it is on disk before any of them apply, so their order does not matter; an `old_string` that matches more than once without `replace_all`, or two targets covering the same lines, rejects the whole batch without writing.
