...
| `/session` `/new`          | 以 self id 切換 session（`d` 刪除）或建立新的；`/session` 清單下方可編輯當前 session 的 `name`／`self id`／`role`（`/session name\|id\|role` 直接開啟）                                                                                                                                             |
...
| `DELETE`       | `/v1/key`         | **local** — 依 `?key=` 刪除 keychain 中單一憑證，並從已記錄的鍵名清單移除。沒有讀取端點：憑證只能寫入與列出名稱 |
| `GET` `POST`   | `/v1/keys`        | **local** — 列出已記錄的憑證名稱，或以 `{key, value}` 寫入 keychain                    |
...
| `GET`    | `/v1/provider/:provider/console` | **local** — 以 302 導向 provider 的主控台：`?page=key` 為 API key 頁、`plan` 為訂閱方案頁；provider 有方案頁時預設 `plan`，否則 `key`；該頁不存在時回 404                                                                                                                                                                                                                |
...
| 檔案       | `find_files`                      | 以目錄、檔名樣式或內容定位（`mode=list\|glob\|search`）；glob 依修改時間由新到舊排列；search 以 `offset`／`limit` 分頁（每頁 256），回傳符合的路徑、次數與前 5 個命中行號（`output=files`），或帶行號的符合行，可加 `context` 前後文（`output=content`）；`multiline` 讓 regex 可跨行比對 |
|            | `read_files`                      | 批次讀取文字、PDF、DOCX、PPTX、CSV 與圖片；預設 2048 行，文字檔被截斷時結尾會標示下一個 `offset`；`around` 讀取文字檔指定行號前後各 `context` 行（預設 20）；同一路徑可在一次呼叫中重複出現                                                                                               |
|            | `edit_file`                       | 建立、修改、移置或還原檔案（`mode=write\|patch\|remove\|restore`）；覆寫或修改既有檔案前，必須在同一次執行中先以 `read_files` 讀過，且檔案之後未在磁碟上被改動                                                                                                                            |
...
|            | `run_command_readonly`            | 同一套沙箱，給檔案工具涵蓋不到的唯讀檢查（`git status`／`log`／`diff`、`du`、`which`）；不需確認且可與其他呼叫並行。讀檔、列目錄、grep 仍走 `read_files`／`find_files`。唯讀只靠工具描述約束與沙箱防護，程式不另外檢查；`cd` 會被拒絕                                                     |
...
|            | `ask_user`                        | 暫停提問。TUI 為 inline 提問：執行不中斷、該次工具呼叫不設逾時，回答後接著跑。其他頻道則中止執行並把狀態寫成 pending 快照，由回答續跑（快照中 args 截 1 KiB、tool result 截 4 KiB）                                                                                                                                                                                                                                                                  |
...
15 個工具會帶完整 schema 送出——`ask_user`、`calculate`、`chat_history`、`edit_file`、`fetch_page`、`find_files`、`find_tools`、`read_files`、`reasoning_guide`、`run_command`、`run_command_readonly`、`run_skill`、`search_web`、`write_result`、`write_todo`；其餘工具初始只送名稱與描述，參數在首次使用時經 `find_tools(mode=search)` 載入，讓初始工具 payload 遠低於完整註冊表。`edit_file` 的 patch 模式每個 target 只接受 `{old_string, new_string}`（另可帶 `replace_all`）；`new_string` 取代 `old_string`，插入則是在 `new_string` 開頭重複 `old_string`。所有 target 都對寫入前的磁碟原始內容比對，因此列出順序不影響結果；`old_string` 在未帶 `replace_all` 時比對到多處，或兩個 target 覆蓋同一段，整批都會拒絕且不寫入。
