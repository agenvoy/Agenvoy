...
description: |
  建立並排程定時觸發的 skill。**所有新增定時／週期任務、提醒、排程通知的請求必須走此 skill**，禁止直接呼叫 schedule(mode=write)（那是 skill 已存在時的時間綁定工具，不該作為新建排程的入口）。
  ...
  **不觸發**（即使訊息含「觸發」「排程」字眼也不 activate）：
  ...
  流程：解析訊息抽出「要做什麼」「何時觸發」→ 缺項用 ask_user 補問 → 生成 skill 檔案至 ~/.config/agenvoy/skills/scheduler/<short>-<hash8>/SKILL.md（無 scheduler- 前綴，hash 用於避免命名衝突）→ 呼叫 schedule(mode=write) 綁定時間 → 回報。
---
...
| 呼叫方式 | `/<name>` 觸發 | `schedule(mode=write, target=task, skill_name=<short>-<hash8>)` / `schedule(mode=write, target=cron, skill_name=<short>-<hash8>)` |

## 成功標準

- 生成檔案: `~/.config/agenvoy/skills/scheduler/<short>-<hash8>/SKILL.md`，frontmatter `name: <short>-<hash8>`（無前綴）
- skill body 描述任務行為、引用具體 tool
- 呼叫 `schedule(mode=write, target=task, time, skill_name=<short>-<hash8>)` 或 `schedule(mode=write, target=cron, time, skill_name=<short>-<hash8>)` 綁定時間成功
...
| 使用者說 | 工具 | `time` 參數 |
|---|---|---|
| `X 分鐘後` | `schedule(mode=write, target=task)` | `+Xm` |
| `X 小時後` | `schedule(mode=write, target=task)` | `+Xh` |
| `今天 X 點`（24h） | `schedule(mode=write, target=task)` | `HH:MM` |
| `明天 / 特定日期 X 點` | `schedule(mode=write, target=task)` | `YYYY-MM-DD HH:MM` |
| `每 X 分鐘` | `schedule(mode=write, target=cron)` | `*/X * * * *` |
| `每小時` | `schedule(mode=write, target=cron)` | `0 * * * *` |
| `每天 X 點` | `schedule(mode=write, target=cron)` | `MM HH * * *` |
| `每週 N`（0=Sun, 1=Mon, ..., 6=Sat） | `schedule(mode=write, target=cron)` | `MM HH * * N` |
| `每月 D 日 X 點` | `schedule(mode=write, target=cron)` | `MM HH D * *` |

決定走 `schedule(mode=write)` target=task（一次性）或 target=cron（週期）。
...
**`scheduler-skill-creator` 與 `scheduler/` 下任何 skill 不算候選** —— 前者是本流程自己、不能遞迴；後者是 scheduler 用內部 skill（透過 `schedule(mode=write)` 綁定觸發、不能用 `/<name>` 從 body 呼叫）。
...
```
schedule(mode=write)(target="task", time="<time_value>", skill_name="<short>-<hash8>")
# 或
schedule(mode=write)(target="cron", time="<cron_expression>", skill_name="<short>-<hash8>")
```
...
- 排程: `schedule(mode=write)` 的回應內容（含下次觸發時間、ID）
...
| full name（檔案／frontmatter／schedule(mode=write) skill_name 用） | `<short>-<hash8>` | `tsmc-stock-watch-a3f9b2c1` |
...
**步驟 2** 正規化：`schedule(mode=write)(target="cron", time="*/5 * * * *", ...)`
...
**步驟 5** `schedule(mode=write)(target="cron", time="*/5 * * * *", skill_name="tsmc-stock-watch-a3f9b2c1")`
...
- **不**跳過步驟 5 的 `schedule(mode=write)` —— skill 建立但沒綁時間 = 排程不會觸發
