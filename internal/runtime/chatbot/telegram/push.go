...
func PushTelegramResult(ctx context.Context, payload exec.PushPayload) {
	...
	if strings.TrimSpace(cleanText) != "" {
		message := cleanText + chatbot.BuildPushFooter(chatbot.Telegram, payload.Duration, payload.OutputElapsed, payload.Model, payload.Quota, payload.Reasoning, payload.Usage)
