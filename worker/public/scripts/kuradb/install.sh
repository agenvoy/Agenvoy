...
ensure_homebrew_darwin() {
  ...
  # arm64 default prefix is /opt/homebrew, intel is /usr/local. Neither is
  # guaranteed to be on PATH in a non-interactive `curl | bash` session, so
  # always probe both and eval shellenv regardless of whether brew was
  # already installed.
  local brew_bin=""
